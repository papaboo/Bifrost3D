...
__inline_all__ float van_der_corput(unsigned int n, unsigned int scramble) {
    ...
    // Reverse bits of n. TODO Use __brev on the GPU.
    n = (n << 16) | (n >> 16);
    ...
    const float value = float((n >> 8) & 0xffffff) / float(1 << 24);
    const float one_minus_epsilon = 0.9999999403953552f;
    return optix::fminf(value, one_minus_epsilon); // TODO If this min is needed, then the function is biased. Check and fix.
}
...
__inline_all__ float sobol2(unsigned int n, unsigned int scramble) {
    ...
    const float value = float((scramble >> 8) & 0xffffff) / float(1 << 24);
    const float one_minus_epsilon = 0.9999999403953552f;
    return optix::fminf(value, one_minus_epsilon); // TODO If this min is needed, then the function is biased. Check and fix.
}
