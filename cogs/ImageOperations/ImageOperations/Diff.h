...
// ------------------------------------------------------------------------------------------------
float ssim(Image reference_image, Image target_image, Image diff_image = Image()) {
    ...
    // Compute the mean and store all the pixel values in floats for faster lookup.
    // Algorithm (2)
    ...
    // Compute the standard deviation of the image. Algorithm (4).
    Vector3d reference_std_dev = {};
    Vector3d target_std_dev = {};
    Vector3d joined_std_dev = {};
    for (unsigned int y = 0; y < height; ++y) {
        for (unsigned int x = 0; x < width; ++x) {
            ...
            joined_std_dev += {ref_d.x * tar_d.x, ref_d.y * tar_d.y, ref_d.z * tar_d.z };
        }
    }
    ...
    joined_std_dev /= width * height;

    // Compute SSIM, algorithm (13)
    double C1 = 0.000001, C2 = 0.0000001;

    Vector3d ssim = (2 * reference_mean * target_mean + C1) * (2 * joined_std_dev + C2) /
