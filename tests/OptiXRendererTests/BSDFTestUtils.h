...
ThinSheetThroughput integrate_over_thin_sheet(std::function<BSDFSample(optix::float3 wo, optix::float3 random_sample)> bsdf_model_sampler,
    ...
    for (unsigned int i = 0; i < path_count; ++i) {
        // Add 1 to the index to avoid index 0 producing a sample with all zeroes, which can generate invalid samples and slightly bias the result.
        auto rng = RNG::ReverseHalton(i + 1, 0);
        ...
        for (unsigned int bounce = 0; bounce < bounce_count && !terminate_ray; ++bounce) {
            ...
            BSDFSample bsdf_sample = bsdf_model_sampler(ray_wo, make_float3(rng.sample4f()));
...
inline ThinSheetThroughput smooth_thin_sheet_reflectance(float cos_theta_o, float medium_ior, optix::float3 transmission_tint) {
    ...
    // The reflected and transmitted throughput of a thin sheet of depends on the reflection and transmission at the initial intersection, R0 and T0,
    ...
    float Ri = specularity; // Could be compute as the specularity of the refracted ray entering the sheet, but it turns out to always be the initial specularity for smooth glass.
