...
GTEST_TEST(ShadingModelUtils, approx_rough_ggx_thin_sheet_RMSE_regression_test) {
    ...
    for (float roughness : { 0.0f, 0.5f, 1.0f }) {
        ...
        for (float medium_IOR : tested_IORs) {
            ...
            for (float cos_theta_o : { 0.3f, 0.5f, 1.0f}) {
                ...
                float energy_loss_adjustment = 1.0f / (expected_throughput.reflected.x + expected_throughput.transmitted.x);
    ...
    float epsilon = 0.025f;
    float3 expected_reflection_error = { 0.2135f, 0.2001f, 0.1973f };
    float3 expected_transmission_error = { 0.2001f, 0.1005f, 0.0503f };
