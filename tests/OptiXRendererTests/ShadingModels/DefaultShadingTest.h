...
GTEST_TEST(DefaultShadingModel, power_conservation) {
    ...
    for (float roughness : { 0.0f, 0.5f, 0.9f })
        for (float cos_theta_o : { 0.1f, 0.4f, 0.7f, 1.0f }) {
            ...
            EXPECT_FLOAT3_EQ_EPS(result.reflectance, make_float3(1, 1, 1), 1e-3f) << "cos_theta: " << cos_theta_o;
        }
}
    ...
    for (float coat_roughness : { 0.0f, 0.5f, 1.0f }) {
        for (float cos_theta : { 0.2f, 0.4f, 0.6f, 0.8f, 1.0f }) {
            ...
            for (float coat : { 0.25f, 0.5f, 0.75f }) {
                ...
                float interpolated_specularity = non_coated_material.get_specularity().x;
                ...
                float3 expdected_rho = lerp(non_coated_rho, coated_rho, coat);
                EXPECT_FLOAT3_EQ_PCT(expdected_rho, interpolated_rho, acceptable_pct_deviation);
            }
        }
    }
}
...
GTEST_TEST(DefaultShadingModel, regression_test) {
    ...
    BSDFResponse bsdf_responses[] = {
        ...
        // Coated plastic
        { 0.019217f, 0.081176f, 0.09605f, 0.016248f }, { 0.019546f, 0.097521f, 0.11623f, 0.228988f },
