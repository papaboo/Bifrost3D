...
public:
...
#ifndef GPU_COMPILATION
    DefaultShading(const Material material, Math::Vector2f texcoord, float abs_cos_theta_o) {
        // Coat
    ...
    DefaultShading(const Material material, float abs_cos_theta_o) {
        // Coat
        float coat_strength = material.get_coat();
        float coat_roughness = material.get_coat_roughness();
        ...
        // Tint and roughness
        auto tint = material.get_tint();
        auto roughness = material.get_roughness();
    ...
    _inline_all_archs_ Math::RGB get_diffuse_tint() const { return m_diffuse_tint; }
