// Bifrost functions for the Oren-Nayar BSDF.
// ---------------------------------------------------------------------------
...
#ifndef _BIFROST_ASSETS_SHADING_BSDFS_ORENNAYAR_H_
#define _BIFROST_ASSETS_SHADING_BSDFS_ORENNAYAR_H_
...
constexpr float constant1_FON = 0.5f - 2.0f / (3.0f * PIf);

_inline_all_archs_ float E_FON_exact(float cos_theta, float roughness, float A, float B) {
...
_inline_all_archs_ float E_FON_exact(float cos_theta, float roughness) {
...
_inline_all_archs_ float E_FON_approx(float cos_theta, float roughness, float A, float B) {
...
_inline_all_archs_ float E_FON_approx(float cos_theta, float roughness) {
    float A = 1.0f / (1.0f + constant1_FON * roughness); // FON A coeff.
    float B = roughness * A; // FON B coeff.
...
_inline_all_archs_ float evaluate(float roughness, Vector3f wo, Vector3f wi, bool exact = false) {
...
_inline_all_archs_ RGB evaluate(RGB albedo, float roughness, Vector3f wo, Vector3f wi, bool exact = false) {
    return albedo * evaluate(roughness, wo, wi, exact);
}

_inline_all_archs_ float uniform_sampling_probability(float roughness, float cos_theta) {
...
_inline_all_archs_ BSDFResponse evaluate_with_PDF(RGB albedo, float roughness, Vector3f wo, Vector3f wi, bool exact = false) {
    auto reflectance = evaluate(albedo, roughness, wo, wi, exact);
    auto PDF = pdf(roughness, wo, wi);
    return { reflectance, PDF };
}

_inline_all_archs_ BSDFSample sample(RGB albedo, float roughness, Vector3f wo, Vector2f random_sample, bool exact = false) {
    float cos_theta = wo.z;
    float uniform_probability = uniform_sampling_probability(roughness, cos_theta);
    float cltc_probability = 1.0f - uniform_probability;

    Distributions::DirectionalSample distribution_sample;
    float cltc_PDF;
    if (random_sample.x <= uniform_probability) {
        random_sample.x = random_sample.x / uniform_probability;
        distribution_sample = Distributions::UniformHemisphere::sample(random_sample); // sample wi from uniform lobe
        cltc_PDF = Distributions::OrenNayerCLTC::PDF(roughness, wo, distribution_sample.direction); // evaluate CLTC PDF at wi
    } else {
        random_sample.x = (random_sample.x - uniform_probability) / cltc_probability;
        distribution_sample = Distributions::OrenNayerCLTC::sample(roughness, wo, random_sample); // sample wi from CLTC lobe
        cltc_PDF = distribution_sample.PDF;
    }
    constexpr float uniform_PDF = Distributions::UniformHemisphere::PDF();

    BSDFSample bsdf_sample;
    bsdf_sample.direction = distribution_sample.direction;
    bsdf_sample.PDF = uniform_probability * uniform_PDF + cltc_probability * cltc_PDF; // MIS PDF of wi
...
#endif // _BIFROST_ASSETS_SHADING_BSDFS_ORENNAYAR_H_