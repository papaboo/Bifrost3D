...
public:
    ...
    GPU_ENABLED Quaternion(T x, T y, T z, T w)
        : x(x), y(y), z(z), w(w) { }

    // Constructs a quaternion from real and imaginary components.
    GPU_ENABLED Quaternion(Vector3<T> v, T w)
        : x(v.x), y(v.y), z(v.z), w(w){}

    // Cast constructor.
    template <typename U>
    GPU_ENABLED Quaternion(const Quaternion<U>& v) : x(T(v.x)), y(T(v.y)), z(T(v.z)), w(T(v.w)) { }

    // The identity quaternion.
    static __always_inline__ GPU_ENABLED Quaternion<T> identity() {
        return Quaternion(0, 0, 0, 1);
    }

    // Create a quaternion describing a rotation in angles around a normalized axis in R^3.
    // http://www.euclideanspace.com/maths/geometry/rotations/conversions/angleToQuaternion/
    static __always_inline__ GPU_ENABLED Quaternion<T> from_angle_axis(T angle_in_radians, Vector3<T> axis) {
    ...
    static inline GPU_ENABLED Quaternion<T> look_in(Vector3<T> direction, Vector3<T> up = Vector3<T>::up()) {
        ...
        if (trace > T(0)) {
            ...
            int j = next[i];
            int k = next[j];
            T s = sqrt((m[i][i] - (m[j][j] + m[k][k])) + T(1.0));
            Vector3<T> imaginary;
            imaginary[i] = s * T(0.5);
            if (s != T(0)) s = T(0.5) / s;
    ...
    //*****************************************************************************
    __always_inline__ GPU_ENABLED bool operator==(Quaternion<T> rhs) const {
    ...
    __always_inline__ GPU_ENABLED Vector3<T> imaginary() const { return { x, y, z }; }

    // The real part of the quaternion.
    __always_inline__ GPU_ENABLED T real() const { return w; }

    // Quaternion multiplication.
    __always_inline__ GPU_ENABLED Quaternion<T> operator*(Quaternion<T> rhs) const {
    ...
    __always_inline__ GPU_ENABLED Quaternion<T>& operator*=(Quaternion<T> rhs) {
    ...
    // Multiplying a vector by a quaternion, i.e. rotating it.
    __always_inline__ GPU_ENABLED Vector3<T> operator*(Vector3<T> rhs) const {
...
template <typename T>
__always_inline__ GPU_ENABLED Quaternion<T> conjugate(Quaternion<T> v) {
    return Quaternion<T>(-v.x, -v.y, -v.z, v.w);
}

// The inverse of a unit quaternion.
template <typename T>
__always_inline__ GPU_ENABLED Quaternion<T> inverse_unit(Quaternion<T> v) {
    return conjugate(v);
}

// Dot product between two quaternions.
template<typename T>
__always_inline__ GPU_ENABLED T dot(Quaternion<T> lhs, Quaternion<T> rhs) {
    return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z + lhs.w * rhs.w;
}

// Computes the magnitude of a quaternion.
template<typename T>
__always_inline__ GPU_ENABLED T magnitude(Quaternion<T> v) {
    return sqrt(dot(v, v));
}

// Returns the input quaternion as normalized quaternion.
template<typename T>
__always_inline__ GPU_ENABLED Quaternion<T> normalize(Quaternion<T> v) {
    T m = magnitude(v);
...
template<typename T>
__always_inline__ GPU_ENABLED Quaternion<T> nlerp(Quaternion<T> from, Quaternion<T> to, T by) {
...
template<typename T>
__always_inline__ GPU_ENABLED bool almost_equal(Quaternion<T> lhs, Quaternion<T> rhs, unsigned short max_ulps = 4) {
...
template<typename T>
__always_inline__ GPU_ENABLED bool almost_equal_rotation(Quaternion<T> lhs, Quaternion<T> rhs, unsigned short max_ulps = 4) {
