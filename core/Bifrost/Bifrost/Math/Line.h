...
struct Line final {
    ...
    Line() = default;
    GPU_ENABLED Line(T slope, T intercept) : slope(slope), intercept(intercept) {}
    template <typename U>
    GPU_ENABLED explicit Line(const Line<U>& l) : slope(T(l.slope)), intercept(T(l.intercept)) {}
    GPU_ENABLED Line(Vector2<T> point0, Vector2<T> point1) {
    ...
    __always_inline__ GPU_ENABLED T evaluate(T x) const { return slope * x + intercept; }

    __always_inline__ GPU_ENABLED T signed_distance(T x, T y) const { return y - evaluate(x); }
    __always_inline__ GPU_ENABLED T signed_distance(Vector2<T> point) const { return point.y - evaluate(point.x); }

    __always_inline__ GPU_ENABLED T distance(T x, T y) const { return abs(signed_distance(x, y)); }
    __always_inline__ GPU_ENABLED T distance(Vector2<T> point) const { return abs(signed_distance(point)); }

    template <typename InputItr>
    __always_inline__ GPU_ENABLED T mse(InputItr begin, InputItr end) const {
...
public:
    struct Fit final {
        const Line<T> line;
        const T mse;
        GPU_ENABLED Fit(Line<T> line, T mse) : line(line), mse(mse) {}
    };
...
public:
    __always_inline__ GPU_ENABLED Vector2f sum() { return m_sum; }
    __always_inline__ GPU_ENABLED T summed_weight() { return m_sum_weight; }

    GPU_ENABLED LineFitter()
        : m_sum(Vector2<T>::zero()), m_sum_sqr(Vector2<T>::zero()), m_sum_xy(T(0)), m_sum_weight(T(0)) {}

    __always_inline__ GPU_ENABLED void add_sample(T x, T y, T weight = 1) {
        m_sum.x += weight * x;
        m_sum.y += weight * y;
    ...
    __always_inline__ GPU_ENABLED void add_sample(Vector2<T> point, T weight = 1) { return add_sample(point.x, point.y, weight); }

    // Fit a line to the samples added.
    __always_inline__ GPU_ENABLED Fit fit() const {
    ...
    template <typename InputItr>
    static __always_inline__ GPU_ENABLED Fit fit(InputItr begin, InputItr end) {
