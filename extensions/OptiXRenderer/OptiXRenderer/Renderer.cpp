...
struct Renderer::Implementation {
    ...
    optix::Material shading_models[(int)ShadingModel::Count];
...
#pragma warning(disable : 4302 4311)
        ...
        { // Auxiliary image visualization setup.
            ...
            context->setRayGenerationProgram(EntryPoints::PrimitiveID, context->createProgramFromPTXFile(rgp_ptx_path, "primitive_id_RPG"));
        }

        { // Setup default material.
            std::string monte_carlo_ptx_path = get_ptx_path(shader_prefix, "MonteCarlo");

            auto create_shading_model = [](optix::Context context, const std::string& ptx_path, const std::string& closest_hit_program) -> optix::Material {
                optix::Material shading_model = context->createMaterial();
                shading_model->setClosestHitProgram(RayTypes::MonteCarlo, context->createProgramFromPTXFile(ptx_path, closest_hit_program));
                shading_model->setAnyHitProgram(RayTypes::Shadow, context->createProgramFromPTXFile(ptx_path, "shadow_any_hit"));
                OPTIX_VALIDATE(shading_model);
            ...
            shading_models[(int)ShadingModel::Transmissive] = create_shading_model(context, monte_carlo_ptx_path, "transmissive_closest_hit");
        ...
        { // Material updates.
            ...
            if (!Materials::get_changed_materials().is_empty()) {
                if (active_material_count < Materials::capacity()) {
                    ...
                    OptiXRenderer::Material* device_materials = (OptiXRenderer::Material*)material_parameters->map();
                    for (Assets::Material material : Materials::get_changed_materials()) {
                        if (material.get_changes().any_set(Materials::Change::Created, Materials::Change::Updated)) {
                        ...
                        if (material.get_changes().is_set(Materials::Change::ShadingModel)) {
                            ...
                            for (MeshModel mesh_model : MeshModels::get_iterable()) {
                                if (mesh_model.get_material() == material) {
                                    mesh_models[mesh_model.get_ID()]->setMaterial(0, new_shading_model);
                                    should_reset_accumulations = true;
                                }
                            }
                        }
        ...
        { // Model updates.
            ...
            for (MeshModel model : MeshModels::get_changed_models()) {
                ...
                if (model.get_changes().contains(MeshModels::Change::Destroyed)) {
                    ...
                    auto shading_model = shading_models[(int)model.get_material().get_shading_model()];
                    optix::GeometryInstance optix_model = create_model(context, model, meshes.data(), shading_model);
                ...
                if (scene_data.get_changes().any_set(SceneRoots::Change::EnvironmentMap, SceneRoots::Change::Created)) {
                    ...
                    if (environment_map != scene.environment.get_environment_map()) {
                        ...
                        if (channel_count(image.get_pixel_format()) == 4) { // TODO Support other formats as well by converting the buffers to float4 and upload.
                            ...
                            TextureSampler environment_sampler = textures[environment_light.get_texture().get_ID()];
#if PRESAMPLE_ENVIRONMENT_MAP
...
void Renderer::set_backend(CameraID camera_ID, Backend backend) {
    ...
    case Backend::ShadingNormalVisualization:
        ...
        break;
    case Backend::PrimitiveIdVisualization:
        camera_state.backend_impl = std::unique_ptr<IBackend>(new SimpleBackend(EntryPoints::PrimitiveID));
        break;
    default:
        camera_state.backend_impl = std::unique_ptr<IBackend>(new SimpleBackend(EntryPoints::Albedo));
