...
__inline_all__ ThinSheetThroughput approx_thin_sheet_reflectance(float abs_cos_theta, float roughness, float ior_i_over_o, optix::float3 transmission_tint) {
    ...
    // Compute the average refracted direction that the opposite side is observed from.
    float3 wo = { sqrt(1 - pow2(abs_cos_theta)), 0.0f, abs_cos_theta };
    float3 refracted_direction;
    bool total_internal_reflection = !refract(refracted_direction, -wo, make_float3(0, 0, 1), ior_i_over_o);
    if (total_internal_reflection)
        return { optix::make_float3(1), optix::make_float3(0) };
    float refracted_cos_theta = abs(refracted_direction.z);
    ...
    float T0 = 1 - R0;
    auto rhoi = DielectricRho::fetch(refracted_cos_theta, roughness, ior_i_over_o);
