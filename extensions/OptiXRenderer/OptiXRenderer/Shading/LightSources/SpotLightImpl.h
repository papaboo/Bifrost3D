...
__inline_all__ PDF pdf(const SpotLight& light, Vector3f lit_position, Vector3f direction_to_light) {
    ...
    if (cos_theta > 0.0f && !is_delta_light(light)) {
        ...
        float t = Intersect::ray_plane(Ray(lit_position, -light.direction), Plane::from_point_normal(light.position, light.direction));
...
__inline_all__ RGB evaluate(const SpotLight& light, Vector3f lit_position, Vector3f direction_to_light) {
    float cos_theta = -dot(light.direction, direction_to_light);

    float normalization = TWO_PIf * (1 - light.cos_angle);
    if (is_delta_light(light))
        normalization *= magnitude_squared(light.position - lit_position);
    else
        normalization *= surface_area(light) * cos_theta;
    RGB radiance = light.power / normalization;

    return (cos_theta > light.cos_angle) ? radiance : RGB::black();
}

__inline_all__ LightResponse evaluate_with_PDF(const SpotLight& light, Vector3f lit_position, Vector3f direction_to_light) {
    return { evaluate(light, lit_position, direction_to_light), pdf(light, lit_position, direction_to_light) };
}

__inline_all__ LightSample sample_radiance(const SpotLight& light, Vector3f lit_position, Vector2f random_sample) {
    using namespace Bifrost::Math;
    ...
    if (is_delta_light(light)) {
        ...
        Matrix3x3f light_to_world = Bifrost::Assets::Shading::compute_TBN(light.direction);
        ...
        if (light.radius > cone_radius_at_intersection && light.cos_angle > spot_light_min_cone_angle_to_sample) {
            ...
            light_sample.distance = Intersect::ray_plane(Ray(lit_position, light_sample.direction_to_light), Plane::from_point_normal(light.position, light.direction));
            ...
            Vector3f sample_position_on_light = lit_position + light_sample.direction_to_light * light_sample.distance;
            Vector3f light_pos_to_sample_pos = sample_position_on_light - light.position;
            if (magnitude_squared(light_pos_to_sample_pos) < pow2(light.radius))
                light_sample.radiance = evaluate(light, lit_position, light_sample.direction_to_light);
        } else {
            // Sample disk and transform to world space.
            auto disk_sample = Distributions::Disk::sample(light.radius, random_sample);
            Vector3f sampled_position = light.position + disk_sample.position * light_to_world;
