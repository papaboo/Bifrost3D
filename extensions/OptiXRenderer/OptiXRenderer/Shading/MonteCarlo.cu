...
__inline_dev__ LightSample sample_single_light(const ShadingModel& material, float3 intersection_point, float3 wo, const TBN& world_shading_tbn, float3 random_sample) {
...
template <class ShadingModel>
__inline_dev__ LightSample reestimated_light_samples(const ShadingModel& material, float3 intersection_point, float3 wo, const TBN& world_shading_tbn) {
    // If there are no lights to sample in the scene then early out.
    if (g_scene.light_count == 0)
        return LightSample::none();

    float4 light_random_base = monte_carlo_payload.rng_sample4f(RngSamplingDimension::NEXT_EVENT_ESTIMATION);
    ...
    for (int s = 0; s < light_sample_count; ++s) {
        ...
        LightSample new_light_sample = sample_single_light(material, intersection_point, wo, world_shading_tbn, light_random_number);
...
__inline_all__ void path_tracing_closest_hit() {
    ...
    float4 bsdf_coverage_random_4f = monte_carlo_payload.rng_sample4f(RngSamplingDimension::BSDF); // Always draw coverage random number to have predictable RNG dimension usage whether the material is a cutout or not.
    ...
    { // Diffuse emissive surface
        monte_carlo_payload.radiance += monte_carlo_payload.throughput * emission * material_parameter.emission * abs(cos_theta);
    }
