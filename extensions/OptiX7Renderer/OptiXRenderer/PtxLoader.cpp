...
int get_compute_capability() {
    // Get the maximally supported compute capability of the CUDA library
    // See https://arnon.dk/matching-sm-architectures-arch-and-gencode-for-various-nvidia-cards/ for matching compute capability and CUDA version
    int max_compute_capability = 86; // Max in CUDA 11.1, which is our minimal requirement
    if constexpr (CUDA_VERSION >= 12080)
        max_compute_capability = 100;
    else if constexpr (CUDA_VERSION >= 11080)
        max_compute_capability = 89;

    int cuda_device_id;
    cudaGetDevice(&cuda_device_id);
    cudaDeviceProp device_properties;
    cudaGetDeviceProperties(&device_properties, cuda_device_id);

    // Get the compute capability of the system.
    int compute_capability = device_properties.major * 10 + device_properties.minor;

    return min(compute_capability, max_compute_capability);
}
...
static string get_ptx_from_cu_source(const string& sample_name, const string& cu_source, const string& name) {
    ...
    std::string compute_arch = "compute_" + std::to_string(get_compute_capability());
    compile_args.push_back("-std=c++17");
    compile_args.push_back("-arch");
    compile_args.push_back(compute_arch.c_str());
