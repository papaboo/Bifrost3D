...
public:
    Implementation(ODevice1& device, const std::filesystem::path& data_directory)
        ...
        { // Setup opaque rendering.
            ...
            m_opaque.shading_models.resize(0);
            for (const char* entry_point : { "default_opaque" , "diffuse_opaque" }) {
                OBlob pixel_shader_blob = m_shader_manager.compile_shader_from_file("ModelShading.hlsl", "ps_5_0", entry_point);
                ID3D11PixelShader* pixel_shader;
                THROW_DX11_ERROR(m_device->CreatePixelShader(UNPACK_BLOB_ARGS(pixel_shader_blob), nullptr, &pixel_shader));
                m_opaque.shading_models.emplace_back(pixel_shader);
            }
        }
        ...
        { // Setup transparent rendering.
            ...
            m_transparent.shading_models.resize(0);
            for (const char* entry_point : { "default_transparent" , "diffuse_transparent" }) {
                OBlob pixel_shader_blob = m_shader_manager.compile_shader_from_file("ModelShading.hlsl", "ps_5_0", entry_point);
                ID3D11PixelShader* pixel_shader;
                THROW_DX11_ERROR(m_device->CreatePixelShader(UNPACK_BLOB_ARGS(pixel_shader_blob), nullptr, &pixel_shader));
                m_transparent.shading_models.emplace_back(pixel_shader);
            }
        }
