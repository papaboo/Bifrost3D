...
public:
    ...
    Renderer(ODevice1& device, const std::filesystem::path& data_directory);
    static IRenderer* initialize(ODevice1& device, const std::filesystem::path& data_directory) { return new Renderer(device, data_directory); }
    ~Renderer();
