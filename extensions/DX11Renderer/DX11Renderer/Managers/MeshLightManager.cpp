...
inline bool is_emissive_model(MeshModel model) {
    // The material alone decides if a model is emissive.
    // Materials with zero emission will scale the emissive vertices to black,
    // while models with no emissive vertices will just use the material emission as is.
...
void MeshLightManager::handle_updates(ID3D11Device1& device) {
    ...
    auto remove_mesh_light = [&](MeshModel model) {
    ...
    auto update_material = [&](MeshLight& light, Material material) {
        ...
        if (material_emission != RGB::black()) {
            ...
            m_lights.erase(light.model_ID.get_index());
            rebuild_combined_buffers = true;
        }
    };
    ...
    for (MeshModel model : MeshModels::get_changed_models()) {
        if (model.get_changes().contains(MeshModels::Change::Destroyed))
            remove_mesh_light(model);

        bool emissive_model = is_emissive_model(model);
        if (model.get_changes().contains(MeshModels::Change::Created) && emissive_model)
    ...
    // Iterate in reverse to support deleting the currently iterated element.
    for (auto light_itr = m_lights.rbegin(); light_itr != m_lights.rend(); ++light_itr) {
        const MeshModel model = light_itr->second.model_ID;
        auto node_change = model.get_scene_node().get_changes();
        if (node_change.contains(SceneNodes::Change::Destroyed))
            remove_mesh_light(model);
        
        if (node_change.contains(SceneNodes::Change::Transform))
            rebuild_combined_buffers = true;
    }
    ...
    if (rebuild_combined_buffers) {
        ...
        for (const auto& light_itr : m_lights)
